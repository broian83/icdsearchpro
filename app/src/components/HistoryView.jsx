  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-[#00B4A4]">
        ...
        <p className="font-medium text-slate-500 dark:text-slate-400">Memuat riwayat pencarian...</p>
      </div>
    );
  }
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Riwayat Pencarian</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">Daftar pencarian dan konsultasi yang pernah Anda lakukan.</p>
          </div>
        </div>
        {history.length > 0 && (
          <button 
      ...
      {history.length === 0 ? (
        <div className="text-center py-16 text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50">
          ...
          <p className="text-lg font-medium text-slate-600 dark:text-slate-300">Belum ada riwayat.</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50 overflow-hidden">
          <div className="divide-y divide-slate-100">
            {history.map((h) => (
              <div key={h.id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:bg-slate-800/50 transition-colors group">
                <div className="flex items-start gap-3 flex-1 min-w-0 cursor-pointer" onClick={() => onSearchHistory && onSearchHistory(h.query, h.search_type)}>
                  <Search className="w-5 h-5 text-slate-400 dark:text-slate-500 mt-0.5 shrink-0" />
                  <div className="min-w-0">
                    <p className="text-slate-800 dark:text-slate-100 font-medium truncate">{h.query}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">
                        {h.search_type}
                      </span>
