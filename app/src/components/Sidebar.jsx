...
export function Sidebar({ isOpen, onClose, onSelectTab }) {
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <div 
        className={`fixed top-0 left-0 h-full w-72 sm:w-80 bg-white z-50 transform transition-transform duration-300 ease-in-out shadow-2xl flex flex-col ${
      ...
      >
        ...
        <div className="p-6 border-b border-slate-100 relative">
          ...
          {isLoggedIn ? (
            <div className="flex items-center gap-4 mt-2">
              <div className="w-12 h-12 rounded-full overflow-hidden bg-slate-100 shrink-0 border-2 border-[#00B4A4]/20 flex items-center justify-center">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-slate-800 truncate">{user?.user_metadata?.full_name || 'Pengguna'}</h3>
            ...
            <div className="mt-4">
              <h3 className="font-bold text-slate-800 mb-2">Masuk ke ICD Search</h3>
              <p className="text-xs text-slate-500 mb-4">Akses Klinik AI dan simpan bookmark di Cloud.</p>
              <button 
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3 flex flex-col gap-1">
          <p className="px-3 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Kamus</p>
          <button 
            onClick={() => { onSelectTab('icd10'); onClose(); }}
            className="w-full flex items-center gap-3 px-3 py-2.5 text-slate-700 hover:bg-slate-50 hover:text-[#00B4A4] rounded-lg transition-colors font-medium text-left"
          >
            <Search className="w-5 h-5 text-slate-400" /> ICD-10
          </button>
          <button 
            onClick={() => { onSelectTab('icd9'); onClose(); }}
            className="w-full flex items-center gap-3 px-3 py-2.5 text-slate-700 hover:bg-slate-50 hover:text-[#00B4A4] rounded-lg transition-colors font-medium text-left"
          >
            <Search className="w-5 h-5 text-slate-400" /> ICD-9
          </button>

          <p className="px-3 text-xs font-bold text-slate-400 uppercase tracking-wider mt-6 mb-2">Fitur Pro</p>
          <button 
            onClick={() => { onSelectTab('case'); onClose(); }}
            className="w-full flex items-center justify-between px-3 py-2.5 text-slate-700 hover:bg-slate-50 hover:text-[#00B4A4] rounded-lg transition-colors font-medium text-left group"
          >
            <div className="flex items-center gap-3">
              <Brain className={`w-5 h-5 ${isLoggedIn ? 'text-[#00B4A4]' : 'text-slate-400'}`} /> 
              Klinik AI {isLoggedIn && <span className="text-xs">✨</span>}
            </div>
            {!isLoggedIn && <Lock className="w-4 h-4 text-slate-300 group-hover:text-slate-400" />}
          </button>
          <button 
          ...
          >
            <div className="flex items-center gap-3">
              <Star className="w-5 h-5 text-slate-400" /> Bookmark Saya
            </div>
            {!isLoggedIn && <Lock className="w-4 h-4 text-slate-300 group-hover:text-slate-400" />}
          </button>
          <button 
          ...
          >
            <div className="flex items-center gap-3">
              <Cloud className="w-5 h-5 text-slate-400" /> Histori Cloud
            </div>
            {!isLoggedIn && <Lock className="w-4 h-4 text-slate-300 group-hover:text-slate-400" />}
          </button>
        </div>
        ...
        <div className="p-4 border-t border-slate-100 flex flex-col gap-1">
          ...
          <button 
            ...
            className="w-full flex items-center gap-3 px-3 py-2 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-lg transition-colors font-medium text-left text-sm"
          >
            <HelpCircle className="w-4 h-4 text-slate-400" /> Bantuan
          </button>
